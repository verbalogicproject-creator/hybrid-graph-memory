...
- C11 is the one confirmed empirical row, and its scope is narrow: the
  disambiguation gate's accept/refuse behaviour was evaluated against a
  preregistered rule (H7) on a held-out split of queries the tuner did not write.
  It says nothing about retrieval *quality*. Do not cite it for C4-C6, and do not
  widen it past "this corpus, this embedder, unreplicated, no blinded assessors"
  without a second corpus. The run lives in `research/disambiguation-gate/`.
