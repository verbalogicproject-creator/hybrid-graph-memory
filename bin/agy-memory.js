...
const mapper_1 = require("../src/ast/mapper");
const doctor_1 = require("../src/cli/doctor");
const dashboard_1 = require("../src/cli/dashboard");
const web_dashboard_1 = require("../src/server/web_dashboard");
...
async function handleInteractiveMenu(engine) {
    printBanner();
    ...
    while (keepRunning) {
        ...
        switch (response.action) {
            case "search": {
                const p = await (0, prompts_1.default)({
                    type: "text",
                    name: "query",
            ...
            case "map:ast": {
                ...
                const mapper = new mapper_1.AstDependencyMapper();
                const content = mapper.generateCtxContent();
                console.log(content);
                break;
            }
            case "doctor": {
                await (0, doctor_1.runDoctor)(engine);
                break;
            }
            case "dashboard": {
                await (0, dashboard_1.runDashboard)(engine);
                break;
            }
            case "serve": {
                (0, web_dashboard_1.serveWebDashboard)(engine);
                break;
            }
        }
    }
...
async function main() {
    ...
    const engine = new engine_1.MemoryEngine();
    await engine.init();
    try {
        if (!command) {
